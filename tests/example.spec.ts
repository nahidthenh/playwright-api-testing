...
test('Create, Update and delete Article', async ({ request }) => {
  ...
  // Update Post
  const updatePostResponse = await request.put(`https://conduit-api.bondaracademy.com/api/articles/${slugId}`, {
    data: {
      "article": {
  ...
  const updatePostResponseJson = await updatePostResponse.json();
  expect(updatePostResponse.status()).toBe(200)
  expect(updatePostResponseJson.article.title).toEqual('Test Post Updated 2026')
  ...
  const articlesResponseJson = await articlesResponse.json();
  expect(articlesResponse.status()).toBe(200)
  expect(articlesResponseJson.articles[0].title).toEqual('Test Post Updated 2026')

  const updateSlugId = updatePostResponseJson.article.slug

  // Delete Post 

  const deletePostResponse = await request.delete(`https://conduit-api.bondaracademy.com/api/articles/${updateSlugId}`, {
    headers: {
      Authorization: authToken
    }
  })
